...
export const TAB_BAR_FLOAT_BOTTOM_GAP_PHONE_IOS = 2;
...
export function tabBarFloatInsets(isNativePhone: boolean, bottomInset: number) {
  ...
  let bottomGap = isNativePhone ? TAB_BAR_FLOAT_BOTTOM_GAP_PHONE : TAB_BAR_FLOAT_BOTTOM_GAP_WEB;
  if (isNativePhone && Platform.OS === 'ios') {
    bottomGap = TAB_BAR_FLOAT_BOTTOM_GAP_PHONE_IOS;
  }
  return {
    horizontal,
    bottomGap,
...
export function tabBarFloatScrollExtra(isNativePhone: boolean): number {
  if (isNativePhone && Platform.OS === 'ios') return TAB_BAR_FLOAT_BOTTOM_GAP_PHONE_IOS;
