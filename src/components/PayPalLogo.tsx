...
export const PAYPAL_BUTTON_BLUE = '#0070BA';
export const PAYPAL_BUTTON_BLUE_PRESSED = '#003087';
export const PAYPAL_WORDMARK_DARK = '#003087';
export const PAYPAL_WORDMARK_LIGHT = '#009CDE';

type Props = {
  height?: number;
  /** Brand two-tone on light backgrounds; white wordmark on PayPal blue buttons. */
  variant?: 'brand' | 'white';
};
...
/** PayPal icon mark (2014), from Wikimedia Commons. */
const MARK_PATHS = [
  {
    fill: '#003087',
...
const MARK_VIEWBOX_WIDTH = 154.728;
const MARK_VIEWBOX_HEIGHT = 190.5;

/** PayPal icon mark (2014). */
export function PayPalMark({ height = 16 }: { height?: number }) {
  const width = (height * MARK_VIEWBOX_WIDTH) / MARK_VIEWBOX_HEIGHT;
...
export function PayPalLogo({ height = 20, variant = 'brand' }: Props) {
  const width = (height * 101) / 32;
  const payFill = variant === 'white' ? '#FFFFFF' : PAYPAL_WORDMARK_DARK;
  const palFill = variant === 'white' ? '#FFFFFF' : PAYPAL_WORDMARK_LIGHT;
  ...
  return (
    ...
    >
      {PAY_PATHS.map((d) => (
        <Path key={d.slice(0, 12)} d={d} fill={payFill} />
      ))}
      {PAL_PATHS.map((d) => (
        <Path key={d.slice(0, 12)} d={d} fill={palFill} />
      ))}
    </Svg>
  );
}
