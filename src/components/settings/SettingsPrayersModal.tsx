...
import type { PrayerId } from '../../lib/prayers/prayers';
...
export type PrayerToggleOption = {
  id: PrayerId;
  label: string;
  hint: string;
  leading: ReactNode;
...
type Props = {
  ...
  title: string;
  subtitle?: string;
  options: readonly PrayerToggleOption[];
  onToggle: (id: PrayerId, next: boolean) => void;
  onClose: () => void;
  isDark: boolean;
  footerNote?: string;
};

/** Multi-select prayer picker — same floating sheet as notifications. */
export function SettingsPrayersModal({
  visible,
  title,
  subtitle,
  options,
                ...
                >
                  ...
                  <View style={styles.textCol}>
                    ...
                    <Text
                      style={[
                        styles.optionHint,
                        { color: selected ? 'rgba(255,255,255,0.82)' : mutedColor },
                      ]}
                      numberOfLines={2}
                    >
                      {option.hint}
